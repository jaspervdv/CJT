...
namespace CJT
{
	...
	void CityCollection::eliminateZeroAreaVerts(json* boundaries, std::vector<int>& eliminatedFaces)
	{
		// check if face list
		if (!boundaries->at(0).is_array()) { return; }
		if (boundaries->at(0).empty()) { return; }
		...
		for (json::iterator obb = boundaries->begin(); obb != boundaries->end(); ++obb)
		{
			...
			for (json::iterator ringObb = faceObject->begin(); ringObb != faceObject->end(); ++ringObb)
			{
				...
				{
					int currentVertIdx = ringObject->at(i);
					CJTPoint currentPoint = indx2VerticesMap_->at(currentVertIdx);
					// get neighbours
					int nextVertIdx = ringObject->at(0);
					if (i + 1 < ringObject->size()) { nextVertIdx = ringObject->at(i + 1); }
					CJTPoint nextPoint = indx2VerticesMap_->at(nextVertIdx);
					int prevVertIdx = ringObject->at(ringObject->size() - 1);
					if (i > 0)  { prevVertIdx = ringObject->at(i - 1);  }
					CJTPoint prevPoint = indx2VerticesMap_->at(prevVertIdx);
					// construct vector
					CJTVector nextVec(currentPoint - nextPoint);
					CJTVector prevVec(prevPoint - currentPoint);

					if (!nextVec.isParallel(prevVec))
					{
	...
	CJTVector::CJTVector(const CJTPoint& other)
	{
		x_ = other.getX();
		y_ = other.getY();
		z_ = other.getZ();
	...
	bool CJTVector::isParallel(const CJTVector& otherVector) const
	{
		double a = y_ * otherVector.getZ() - z_ * otherVector.getY();
		double b = z_ * otherVector.getX() - x_ * otherVector.getZ();
		double c = x_ * otherVector.getY() - y_ * otherVector.getX();

		if (abs(a) > precision) { return false; }
		if (abs(b) > precision) { return false; }
		if (abs(c) > precision) { return false; }

		return true;
	}

	double CJTVector::magnitude() const
	{
	...
	void ObjectTransformation::setScale(double scale)
	{
		xScale_ = scale;
		yScale_ = scale;
		zScale_ = scale;
	...
	void AppearanceObject::addMaterial(MaterialObject obb) {
		...
		obb.setIdx(matIdx);
		materials_.emplace_back(std::make_shared<MaterialObject>(obb));
	}
	...
	bool CJTPoint::isEqual(const CJTPoint& other, const double& precision) const
	{
		if (abs(x_ - other.getX()) > precision) { return false; }
		if (abs(y_ - other.getY()) > precision) { return false; }
		if (abs(z_ - other.getZ()) > precision) { return false; }
	...
	CJTPoint CJTPoint::operator-(CJTPoint other) const
	{
		return CJTPoint(
			x_ - other.getX(),
			y_ - other.getY(),
	...
	)
	{
		...
		pointOfContact_ = std::make_shared<PointOfContactObject>(pointOfContact);
	...
	{
		geometry_.emplace_back(std::make_shared<GeoObject>(geom));
		hasGeo_ = true;
	}

	std::vector<std::shared_ptr<GeoObject>> CityObject::getGeoObjectsPtr()
	{
		return geometry_;
	}
	...
	std::vector<std::shared_ptr<GeoObject>> CityObject::getGeoObjectsPtr(const std::string& lod)
	{
		std::vector<std::shared_ptr<GeoObject>> geoObjectList;
		for (size_t i = 0; i < geometry_.size(); i++)
		{
			if (geometry_[i]->getLoD() == lod)
			{
	...
	void CityObject::addParent(std::shared_ptr<CityObject> parentObject)
	{
	...
	void CityObject::removeParent(std::shared_ptr<CityObject> removeableObject)
	{
	...
	void CityObject::addChild(std::shared_ptr<CityObject> childObject)
	{
	...
	void CityObject::removeChild(std::shared_ptr<CityObject> removeableObject)
	{
	...
	void CityCollection::fetchPoints(const json& pointJson)
	{
		...
		for (auto point = pointJson.begin(); point != pointJson.end(); ++point)
		{
			...
			CJTPoint currentCJP = CJTPoint((double)coord[0] * scaler[0], (double)coord[1] * scaler[1], (double)coord[2] * scaler[2]);

			indx2VerticesMap_->emplace(static_cast<int>(indx2VerticesMap_->size()), currentCJP);
			if (vertices2IndxMap_->find(currentCJP) == vertices2IndxMap_->end())
			{
				vertices2IndxMap_->emplace(currentCJP, static_cast<int>(vertices2IndxMap_->size()));
			}
		}
		return;
	}

	std::map<std::string, std::shared_ptr<CityObject>> CityCollection::fetchCityObjects(const json& cityObjects)
	{
		std::map<std::string, std::shared_ptr <CityObject>> collection;
		...
		for (auto cityObject = cityObjects.begin(); cityObject != cityObjects.end(); ++cityObject)
		{
			...
			{
				collection.emplace(objectName, std::make_shared<CityObject>(CCityObject));
				continue;
			}
			...
			collection.emplace(objectName, std::make_unique<CityObject>(CCityObject));
		}
		return collection;
	}
	...
	bool CityCollection::parseJSON(const std::string& filePath, bool silent)
	{
		...
		objectTransformation_ = std::make_shared<ObjectTransformation>(fetchTransformation(completeData["transform"]));
		fetchPoints(completeData["vertices"]);
		//TODO: fix this

		if (!isSilent_)
		{
			std::cout << "Loaded " << indx2VerticesMap_->size() << " vertices of which " << vertices2IndxMap_->size() << " unique vertices" << std::endl;
		}
		...
		appearance_ = std::make_shared<AppearanceObject>(fetchAppearance(completeData["appearance"]));
		...
		if (!isSilent_)
		{
			if (vertices2IndxMap_->size() != 0 && cityObjects_.size() != 0)
			{
		...
		version_ = std::make_shared<std::string>(completeData["version"]);
		metaData_ = std::make_shared<metaDataObject>(metaDataObject(completeData["metadata"]));
	...
	bool CityCollection::dumpJson(const std::string& filePath)
	{
		...
		// offload vertices
		std::vector<std::array<int, 3>> vertList(indx2VerticesMap_->size());

		for (const auto& [indx, currentVert] : *indx2VerticesMap_)
		{
		...
		for (std::pair<std::string, std::shared_ptr<CityObject>> cityObjectPair : cityObjects_)
		{
			auto currentObject = cityObjectPair.second;
			std::string objectName = currentObject->getName();
			std::string objectType = buildingTypeToString(currentObject->getType());
			std::vector<std::shared_ptr<GeoObject>> geoObjectList = currentObject->getGeoObjectsPtr();
		...
		int c = 0;
		for (std::shared_ptr<MaterialObject> materialObject : getMaterialsPtr())
		{
			json materialJson;
		...
		c = 0;
		for (std::shared_ptr<TextureObject> textureObject : getTexturesPtr())
		{
			json textureJson;
	...
	std::vector<std::shared_ptr<CityObject>> CityCollection::getCityObjectPtr(const std::vector<std::string>& obNameList)
	{
		std::vector<std::shared_ptr<CityObject>> cityObjectList;
		for (size_t i = 0; i < obNameList.size(); i++)
		{
			if (!containsCityObject(obNameList[i])) { continue; }
	...
	std::vector<std::shared_ptr<CityObject>> CityCollection::getCityObjectTypedPtr(const Building_Type& typeName)
	{
		std::vector<std::shared_ptr<CityObject>> cityObjectList;
		for (auto cityObject: cityObjects_)
		{
			cityObjectList.emplace_back(cityObject.second);
		}
		return cityObjectList;
	}
	...
	{
		std::vector<CJTPoint> pointList;
		pointList.reserve(indx2VerticesMap_->size());
		for (size_t i = 0; i < indx2VerticesMap_->size(); i++)
		{
			pointList.emplace_back(indx2VerticesMap_->at(i));
		}
		return pointList;
	}
	...
	{
		auto it = vertices2IndxMap_->find(point);
		if (it != vertices2IndxMap_->end())
		{
			if (abs(point.getX() - it->first.getX()) > precision) { it->first.print(); std::cout << "\n"; }
			if (abs(point.getY() - it->first.getY()) > precision) { it->first.print(); std::cout << "\n"; }
			if (abs(point.getZ() - it->first.getZ()) > precision) { it->first.print(); std::cout << "\n"; }

			return it->second;
		}
		int location = static_cast<int>(vertices2IndxMap_->size());
		
		vertices2IndxMap_->emplace(point, location);
		indx2VerticesMap_->emplace(static_cast<int>(indx2VerticesMap_->size()), point);
	...
	void CityCollection::cullUnreferencedVerices()
	{
		...
		// a list that counts the occurances of coords
		PointMap pointOccuranceMap = *vertices2IndxMap_;
		for (auto& [key, value] : pointOccuranceMap) {
			value = 0;
		}
		...
		for (auto obb = cityObjects_.begin(); obb != cityObjects_.end(); ++obb)
		{
			...
			for (size_t i = 0; i < curentGeoObjects.size(); i++)
			{
				std::shared_ptr<GeoObject> currentGeoObject = curentGeoObjects[i];
				json boundaries = currentGeoObject->getBoundaries();
				std::vector<int> indxList = getFlatVerts(boundaries);

				for (int indx : indxList)
				{
					CJTPoint currentPoint = indx2VerticesMap_->at(indx);
					pointOccuranceMap[currentPoint] = 1;
				}
			}
		}

		bool process = false;
		for (const auto& [key, value] : pointOccuranceMap)
		{
			if (value == 0) 
			{ 
				process = true;
		...
		if (!process)
		{
			if (!isSilent_)
			{
				std::cout << "No unreferenced vertices to correct" << std::endl;
			}
			return;
		}

		vertices2IndxMap_->clear();
		int counter = 0;
		for (const auto& [key, value] : pointOccuranceMap)
		{
			if (value == 0) { continue; }
			vertices2IndxMap_->emplace(key, counter);
			counter++;
		}
	...
	void CityCollection::cullCollapesedRings()
	{
		for (auto obb = cityObjects_.begin(); obb != cityObjects_.end(); ++obb)
		{
			CityObject currentCityObject = *obb->second;
			...
			for (size_t i = 0; i < curentGeoObjects.size(); i++)
			{
				...
				std::vector<int> eleminatedFace;
				eliminateZeroAreaVerts(&boundaries, eleminatedFace);
